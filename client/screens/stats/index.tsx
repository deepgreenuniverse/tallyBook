        ...
        <ScrollView style={styles.flex} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerBtns}>
              <TouchableOpacity style={[styles.themeBtn, { backgroundColor: colors.cardBg }]} onPress={toggleTheme}>
                <Ionicons name={theme === 'dark' ? 'sunny' : 'moon'} size={20} color={colors.textSecondary} />
              </TouchableOpacity>
              <TouchableOpacity style={[styles.budgetBtn, { backgroundColor: colors.cardBg }]} onPress={() => {
          ...
          <View style={styles.totalCard}>
            <BlurView intensity={theme === 'dark' ? 40 : 15} tint={theme} style={[styles.totalBlur, { backgroundColor: colors.cardBg }]}>
              <Text style={[styles.totalLabel, { color: colors.textSecondary }]}>累计支出</Text>
              <Text style={[styles.totalValue, { color: colors.danger }]}>¥{formatAmount(total)}</Text>
              <Text style={[styles.totalCount, { color: colors.textMuted }]}>{records.length} 笔</Text>
            </BlurView>
          </View>
          ...
          {budget > 0 && (
            <View style={styles.budgetCard}>
              <BlurView intensity={theme === 'dark' ? 30 : 10} tint={theme} style={[styles.budgetBlur, { backgroundColor: colors.cardBg }]}>
                <View style={styles.budgetHeader}>
                  <Text style={[styles.budgetLabel, { color: colors.textMuted }]}>预算</Text>
                  <Text style={[styles.budgetAmount, { color: colors.text }]}>¥{formatAmount(budget)}</Text>
                </View>
                <View style={[styles.progressBg, { backgroundColor: theme === 'dark' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' }]}>
                  <View style={[styles.progressFill, {
                    width: `${percent}%`,
                    backgroundColor: percent > 90 ? colors.danger : percent > 70 ? colors.warning : colors.success,
                  }]} />
                </View>
                <View style={styles.budgetFooter}>
                  <Text style={[styles.remainingText, { color: remaining >= 0 ? colors.success : colors.danger }]}>
                    {remaining >= 0 ? `剩余 ¥${formatAmount(remaining)}` : `超支 ¥${formatAmount(Math.abs(remaining))}`}
                  </Text>
                  <Text style={[styles.percentText, { color: colors.textMuted }]}>{percent.toFixed(0)}%</Text>
                </View>
              </BlurView>
            </View>
          )}
          ...
          {categoryStats.length > 0 && (
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: colors.text }]}>分类支出</Text>
              <View style={styles.categoryList}>
                {categoryStats.map((cat) => (
                  <View key={cat.id} style={[styles.categoryItem, { backgroundColor: colors.cardBg }]}>
                    <View style={[styles.catDot, { backgroundColor: cat.color }]} />
                    <Text style={[styles.catName, { color: colors.textSecondary }]}>{cat.name}</Text>
                    <Text style={[styles.catAmount, { color: colors.text }]}>¥{formatAmount(cat.total)}</Text>
                  </View>
                ))}
              </View>
            </View>
          ...
          <View style={styles.section}>
            ...
            {records.length === 0 ? (
              <View style={styles.emptyCard}>
                <BlurView intensity={theme === 'dark' ? 20 : 10} tint={theme} style={[styles.emptyBlur, { backgroundColor: colors.cardBg }]}>
                  <Ionicons name="receipt-outline" size={40} color={colors.textMuted} />
                  <Text style={[styles.emptyText, { color: colors.textMuted }]}>暂无记录</Text>
                </BlurView>
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  budgetBtn: { width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  ...
  totalValue: { fontSize: 42, fontWeight: '700', letterSpacing: -1 },
  totalCount: { fontSize: 13, marginTop: 6 },
  ...
  categoryList: { gap: 8 },
  categoryItem: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 14, borderRadius: 10 },
  ...
  modalCancel: { flex: 1, paddingVertical: 14, alignItems: 'center', borderRadius: 12 },
